...
deribit_parser - асинхронный сервис на FastAPI для парсинга цен индексов биржи Deribit и хранения их в PostgreSQL. Приложение позволяет: регулярно (по заданному расписанию) собирать данные о ценах, хранить исторические значения, получать цены через REST API, запускаться в контейнерах (Docker). 
...
Проект построен с рядом архитектурных решений, призванных обеспечить устойчивость, модульность и тестируемость:
